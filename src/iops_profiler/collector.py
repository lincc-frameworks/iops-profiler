...
def launch_helper_via_osascript(helper_script_path):
...
def measure_macos_osascript(shell, code, collect_ops=False):
    """Measure IOPS on macOS using fs_usage via osascript
    
    Args:
        shell: IPython shell for running code
    ...
    try:
        ...
        script_content = create_helper_script(pid, output_file, control_file)
        ...
        helper_proc = launch_helper_via_osascript(helper_script)
        ...
        if os.path.exists(output_file):
            with open(output_file, 'r') as f:
                for line in f:
                    if collect_ops:
                        op = parse_fs_usage_line(line, collect_ops=True)
                        if op:
                        ...
                        op_type, bytes_transferred = parse_fs_usage_line(line)
                        if op_type == 'read':
                            read_count += 1
...
def measure_linux_strace(shell, strace_pattern, io_syscalls, code, collect_ops=False):
        ...
        if os.path.exists(output_file):
            try:
                ...
                with open(output_file, 'r', errors='ignore') as f:
                    for line in f:
                        if collect_ops:
                            op = parse_strace_line(line, strace_pattern, io_syscalls, collect_ops=True)
                            if op:
                                operations.append(op)
                                if op['type'] == 'read':
                                    read_count += 1
                            ...
                            op_type, bytes_transferred = parse_strace_line(line, strace_pattern, io_syscalls)
                            if op_type == 'read':
                                read_count += 1
                                read_bytes += bytes_transferred
                            elif op_type == 'write':
...
def measure_linux_windows(shell, code):
    ...
    try:
        ...
        raise RuntimeError(f"psutil.Process.io_counters() not supported on {sys.platform}")
...
def measure_systemwide_fallback(shell, code):
